                ...
                <v-row>
                    ...
                    <v-col sm="4" cols="12" class="py-0">
						<div class="report_export">
							<label class="label">{{ $t("reporting_language") }}:</label>
...
export default {
	...
	methods: {
		...
		onGridChanges() {
			let grid = this.$refs.gridSFP.kendoWidget();
		...
		search() {
			...
			if (this.asOfDate && this.location.id !== "") {
				...
				balanceSheetHandler
					.getByBusinessUnit({
						params: {
							...
							as_of_date: kendo.toString(new Date(asOfDate), "yyyy-MM-dd") + "T23:59:59.999Z",
						},
					}, this.location.id)
					.then((res) => {
						if (res) {
        ...
        bindData(){
            ...
            this.statements.forEach((value) => {
                ...
                if (entry.groupNature.toLowerCase() === "asset") {
                    ...
                    if(entry.name === "Profit For The Period"){
                        for (let i = 0; i < 12; i++) {
                            let propertyName = NumberInWord[i];
                            
                            entry[propertyName] = entry[propertyName] * -1;
                        }
                    }
