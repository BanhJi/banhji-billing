...
export default {
	...
	methods: {
		...
		async bindData() {
			...
			this.budget.lines.forEach((value) => {
				...
				if(clazz.clazz.id !== ""){
					...
					/* Parent */
					let parentLine = new ClassBudgetPerformanceModel(clazz.clazz);
				...
				if(clazz.classType.id !== ""){
					...
					/* Parent */
					let parentLine = new ClassBudgetPerformanceModel(clazz.classType);
				...
				if(clazz.classGroup.id !== ""){
					...
					/* Parent */
					let parentLine = new ClassBudgetPerformanceModel(clazz.classGroup);
				...
				if(clazz.classSubGroup.id !== ""){
					...
					/* Parent */
					let parentLine = new ClassBudgetPerformanceModel(clazz.classSubGroup);
				...
				if (entry.actual_ytd !== 0) {
					entry.ytd_variance = (entry.budget_ytd - entry.actual_ytd) / entry.budget_ytd;
				}
		...
		generateColumn() {
			...
			/* Header template */
			let ytdHeaderTmpl = `<div>${kendo.toString(new Date(this.fiscalDate.start_date), 'dd-MM-yyyy')} - ${lastDayOfMonth}</div>`;
			...
			let columns = [
				...
				{
					field: "actual_ytd",
					headerTemplate: `<div>${i18n.t("year_to_date_actual")}</div>` + ytdHeaderTmpl,
					format: "{0:n}",
					width: 200,
				...
				{
					field: "budget_ytd",
					headerTemplate: `<div>${i18n.t("year_to_date_budget")}</div>` + ytdHeaderTmpl,
					format: "{0:n}",
					width: 200,
					headerAttributes: subHeaderAttributes,
					attributes: attributes,
				...
				{
					field: "ytd_variance",
					headerTemplate: `<div>${i18n.t("year_to_date_variance_%")}</div>` + ytdHeaderTmpl,
					format: "{0:p}",
					width: 200,
					headerAttributes: subHeaderAttributes,
					attributes: attributes,
