<template>
  <v-row>
    <v-col
      ...
      cols="12"
      :class="
        appType != 'npo' && appType != 'Public Sectors' ? 'grayBg px-6' : ''
      "
    >
      <v-card
        color="white"
        :class="
          appType != 'npo' && appType != 'Public Sectors'
            ? 'pa-3 no_border'
            : ''
        "
        elevation="0"
      >
        ...
        <v-row class="mt-0">
          <v-col sm="6" cols="12" class="pt-0">
            ...
            >
              <h3 class="text-left flex-1 text-uppercase font_12 py-0">
                {{ $t("no_of_account_group") }}
              </h3>
              <h3 class="text-right flex-1 font_20">
                {{
                  numberAccountGroup
                    ? Number(numberAccountGroup).toLocaleString()
                    : 0
                }}
              </h3>
            </v-card>
          ...
          <v-col sm="6" cols="12" class="pt-0">
            ...
            >
              <h3 class="text-left flex-1 text-uppercase font_12 py-0">
                {{ $t("no_of_account_group_with_negative") }}
              </h3>
              <h3 class="text-right flex-1 font_20">
                {{
                  numberAccNegBalance
  ...
  methods: {
    ...
    bindData() {
      ...
      this.trialBalance.tbdetails.forEach((value) => {
        ...
        let id = value.account.account_group.code;
        if (entries[id]) {
          entries[id]["dr"] += dr;
          entries[id]["cr"] += cr;
        } else {
          entries[id] = {
            uuid: value.account.account_group.uuid,
            number: value.account.account_group.code,
            name: value.account.account_group.name,
            dr: dr,
            cr: cr,
          };
        }
      ...
      this.numberAccountGroup = this.trialBalances.length;
