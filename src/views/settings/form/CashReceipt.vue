      ...
      <v-row>
        <v-col sm="12" cols="12" class="pt-0">
         ...
         <div style="height: 42px;" class="mb-2">
              <v-btn color="third" class="text-capitalize white--text float-right" @click="EdtiForm(2)">
                {{ $t("edit") }}
              </v-btn>
              <v-btn
                color="third"
                class="text-capitalize white--text float-right mx-3"
                @click="_print(4)"
                >{{ $t("preview") }}</v-btn
              >
              <v-btn
...
import { CashReceipt, print } from "@/form/Sale.js";
...
let def = {
  title: {
    en: { name: "Cash Receipt", status: true },
    kh: { name: "បង្កាន់ដៃសាច់ប្រាក់", status: true },
  },
...
export default {
  ...
  data() {
    return {
      ...
      pk: "",
      invoice_setting: def,
      form_invoice: CashReceipt(this.items, "id_23e", def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then(async (res) => {
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = await CashReceipt(
              this.items,
