			...
			<v-card color="white" class="pa-3 no_border" elevation="0">
				<v-row>
					<v-col sm="12" cols="12" class="pt-0 pb-0">
						<h2 class="mb-0 font_20">{{ $t("expenses_report") }}</h2>

						<p class="mb-0">
							{{ $t("expenses_report_desc") }}
						</p>
					</v-col>
...
export default {
	...
	data: () => ({
		...
		groupDefinition: [],
		aggregateDefinition: [
			{field: "amount_due", aggregate: "sum"},
		],
		// LoadingMe
		showLoading : false,
	}),
	methods: {
        ...
        numberTemplate: (dataItem) => {
            return Helper.linkTemplate(dataItem.number, `expenses/view/${dataItem.uuid}`);
        },
		// Use For Template
        useForTmpl: (dataItem) => {
			let txt = '';
