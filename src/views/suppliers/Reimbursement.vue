	...
	methods: {
		...
		async bindData() {
			...
			this.segment = new SegmentModel(
				segments.find((i) => i.id === this.reimbursement.segment_uuid)
			);
		...
		onProjectChanges() {
			let txns = this.clearingTransactions.filter(
				(item) => item.project_uuid === this.project.id
			);
			this.clearingTransactionGrouped = Helper.groupExpenseByEmployee(txns);
		},
		// Auto Calculating
		...
		goBackToViewPage() {
			let reimbursement = new ReimbursementModel(this.reimbursement);
			this.$router.replace({
				name: "Reimbursement View",
				params: { initReimbursement: reimbursement },
			});
		},
	},
	watch: {
