      ...
      <v-card color="white" class="pa-3 no_border" elevation="0">
        <v-row>
          <v-col sm="12" cols="12" class="pt-0">
            <h2 class="mb-0 font_20">{{ $t("payment_vouchers") }}</h2>
            <p class="mb-0">{{ $t("payment_vouchers_desc") }}</p>
          </v-col>
        </v-row>
        <v-row class="mt-0">
        ...
        <v-row class="mt-0">
          ...
          <v-col sm="4" cols="12" class="pt-0">
            <v-card
              outlined
              dense
              class="pa-3 no_border rounded-4 black--text"
              color="grayBg"
              height="60px"
            >
              <h3 class="text-left text-uppercase font_12 flex-1">
              ...
              <h3 class="text-right font_20 flex-1">
                {{ numberFormat(collectedInvoiceOverDue) }}
              </h3>
            </v-card>
          </v-col>
        </v-row>
          ...
          billingHandler.txnCashPayment(strFilter).then((res) => {
            if (res.data.statusCode === 200) {
              ...
              if (this.transactions.length > 0) {
                ...
                this.collectedAmount = kendo.toString(
                  results.cash,
                  `${this.decimalFormat}`
                );
